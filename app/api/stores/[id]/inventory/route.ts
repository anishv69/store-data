...
export async function GET(_: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const rows = await prisma.inventory.findMany({ where: { storeId: id }, include: { product: true }, orderBy: { product: { name: "asc" } } });
  return NextResponse.json(rows.map((row) => ({ ...row, product: { ...row.product, price: Number(row.product.price) } })));
}
